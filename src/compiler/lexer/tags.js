import tokenizeAttrs from 'attrs'
import {
...
export const tokenizeStartTag = ({ input, cursor, tokens }) => {
  ...
  if (FULL_STRING_OF_ALL_ATTRS_WITHIN_TAG !== '') {
    if (IS_VOID_ELEMENT) {
      const FOR_VOID_ELEMENT = FULL_STRING_OF_ALL_ATTRS_WITHIN_TAG.slice(0, -1)
      ...
      attrs = tokenizeAttrs({ FULL_STRING_OF_ALL_ATTRS_WITHIN_TAG, attrs })
    }
  }
