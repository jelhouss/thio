...
const INVALID_HTML5_DOCTYPE_DECLARATION = 'INVALID_HTML5_DOCTYPE_DECLARATION';
...
/**
 *
 * Handling HTML doctype; skip it
 */
describe('HTML 5 Doctype', function() {
  const skippedTree = { tag: 'ROOT', children: [] };

  context('When there is a valid HTML5 doctype', function() {
    describe('#thio("<!DOCTYPE html>")', function() {
      it('Should skip the doctype declaration', function() {
        thio('<!DOCTYPE html>').should.be.fulfilledWith(skippedTree);
      });
    });
  });

  context(
    'When the doctype declaration is not well written (more than one <space>)',
    function() {
      describe('#thio("<!DOCTYPE  html>")', function() {
        it('Should throw a TypeError of INVALID_HTML5_DOCTYPE_DECLARATION', function() {
          thio('<!DOCTYPE  html>').should.be.rejectedWith(TypeError, {
            message: INVALID_HTML5_DOCTYPE_DECLARATION
          });
        });
      });
    }
  ...
  context(
    'When the doctype declaration is not HTML5 compatible (e.g. HTML 4.01)',
    function() {
      describe('#thio("<!DOCTYPE  html PUBLIC "-//W3C//DTD HTML 4.01//EN">")', function() {
        it('Should throw a TypeError of INVALID_HTML5_DOCTYPE_DECLARATION', function() {
          thio(
...
describe('Comments', function() {
  ...
  context('When there is a comment in the beginning of HTML', function() {
    describe('#thio("<!-- comment --><p>greeting</p>")', function() {
      it('Should skip the comment', function() {
        thio('<!-- comment --><p>greeting</p>').should.be.fulfilledWith(tree);
      });
    });
  });

  context('When there is a comment in the end of HTML', function() {
    describe('#thio("<p>greeting</p><!-- comment -->")', function() {
      it('Should skip the comment', function() {
        thio('<p>greeting</p><!-- comment -->').should.be.fulfilledWith(tree);
      });
    });
  });

  context('When there is a comment in the middle of HTML', function() {
    describe('#thio("<!-- <p>greeting</p>--><p>greeting<!-- bla bla --></p>")', function() {
      it('Should skip the comment', function() {
        thio(
          '<!-- <p>greeting</p>--><p>greeting<!-- bla bla --></p>'
        ).should.be.fulfilledWith(tree);
      });
    });
  });

  context(
    'When there is an open comment in HTML (without a close tag)',
    function() {
      describe('#thio("<!-- <p>greeting</p>")', function() {
        it('Should skip the comment, and return nothing', function() {
          thio('<!-- <p>greeting</p>').should.be.fulfilledWith(skippedTree);
        });
      });
    }
  );
